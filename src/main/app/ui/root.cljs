(ns app.ui.root
  (:require [app.model.session :as session]
            [app.ui.experiments :refer [Experiments]]
            [app.ui.tic-tac-toe :refer [TicTacToeField]]
            [com.fulcrologic.fulcro-css.css :as css]
            [com.fulcrologic.fulcro.algorithms.form-state :as fs]
            [com.fulcrologic.fulcro.components :as comp :refer [defsc]]
            [com.fulcrologic.fulcro.dom :as dom :refer [button div h3 p]]
...
(dr/defrouter TopRouter [this props]
  {:router-targets [Main Signup SignupSuccess TicTacToeField Experiments Settings]})
  ...
  (let [current-tab (some-> (dr/current-route this this) first keyword)]
    (div :.ui.container
         (div :.ui.secondary.pointing.menu
              ...
              (dom/a :.item {:classes [(when (= :tic-tac-toe current-tab) "active")]
                             :onClick (fn [] (dr/change-route this ["tic-tac-toe"]))} "TicTacToe")
              (dom/a :.item {:classes [(when (= :experiments current-tab) "active")]
                             :onClick (fn [] (dr/change-route this ["experiments"]))} "Experiments")
